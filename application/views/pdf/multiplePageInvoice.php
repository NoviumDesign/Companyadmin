...
function template($content, $header, $footer) {
	return '
	<dimensions:595 842; padding:35>
		' . $header . '
		<dimensions:525 507; margin-top:25;>' . $content . '</>
		' . $footer . '
	</>
	';
}
...
$paymentInfo = '
	...
	<dimensions:525 106; margin-top:-151; padding:0 10 0 10;>
...
' . $invoiceInfo  . '
	...
	' . $itemsHeader . '
		...
		' . implode('', array_slice($_items, 0, 22)) . '
		</>
	</>
';
$this->document[] = template($firstPage, $header, $footer);



if (count($_items) > 36) {
	$at = 22;
	$numMiddlePages = ceil((count($_items) - 36)/26);

	for ($i=0; $i < $numMiddlePages; $i++) {
		$middlePage = '
			<dimensions:525 451;>
			' . $itemsHeader . '
				<dimensions:525 270; margin-top:-13;>
				' . implode('', array_slice($_items, $at, 26)) . '
				</>
			</>
		';
		$this->document[] = template($middlePage, $header, $footer);
...
if (count(array_slice($_items, $at, $at))) {
	$lastPage = '
		<dimensions:525 451;>
		' . $itemsHeader . '
			<dimensions:525 270; margin-top:-13;>
			' . implode('', array_slice($_items, $at, 14)) . '
			</>
		' . $paymentInfo . '
		</>
	';
	...
	$lastPage = '
		<dimensions:525 451;>
			<dimensions:525 105; margin-top:50>
				' . $sum . '
			</>
			<dimensions:525 106; padding:0 10 0 10;>
				<dimensions: 505 ' . ($invoice['discount'] > 0? 56: 56+15) . '; margin-top:' . ($invoice['discount'] > 0? 15: 0) . '; font-size:10>' .
					strip_tags(html_entity_decode(ucfirst($invoice['notes']), ENT_QUOTES, 'UTF-8')) . '
				</>
...
$this->document[] = template($lastPage, $header, $footer);